...
public class GrievanceServiceImpl implements GrievanceService {
    ...
    @Override
    public Grievance markStepCompletedByUser(String grievanceId, String step) {
        Grievance grievance = grievanceRepository.findById(grievanceId)
                .orElseThrow(() -> new RuntimeException("Grievance not found"));

        // Validate the step
        if (!grievance.getResolutionSteps().contains(step)) {
            throw new IllegalArgumentException("Step not part of resolution steps.");
        }

        // Add to completedSteps if not already present
        if (!grievance.getCompletedSteps().contains(step)) {
            grievance.getCompletedSteps().add(step);
        }

        // Check if all steps are completed
        if (grievance.getCompletedSteps().containsAll(grievance.getResolutionSteps())) {
            grievance.setStatus(GrievanceStatus.RESOLVED);
            grievance.setUserApprovedResolution(true);
        } else {
